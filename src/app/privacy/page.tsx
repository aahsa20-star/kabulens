...
const sections = [
  ...
  },
  {
    title: "8. 広告の配信について",
    paragraphs: [
      "当サイトは、第三者配信の広告サービス（Google AdSense）を利用しています。このような広告配信事業者は、ユーザーの興味に応じた商品やサービスの広告を表示するため、当サイトや他のサイトへのアクセスに関するCookie（氏名、住所、メールアドレス、電話番号は含まれません）を使用することがあります。",
      "Googleが広告Cookieを使用することにより、ユーザーがそのサイトや他のサイトにアクセスした際の情報に基づいて、Googleやそのパートナーが適切な広告をユーザーに表示できます。ユーザーはGoogleの広告設定（https://www.google.com/settings/ads）でパーソナライズ広告を無効にすることができます。",
    ],
  },
  {
    title: "9. アフィリエイトプログラムについて",
    paragraphs: [
      "当サイトは、アフィリエイトプログラムを利用しています。具体的には、A8.net等のASP（アフィリエイト・サービス・プロバイダ）を通じて、証券会社・金融サービス等の広告を掲載し、利用者が広告を経由してサービスに申し込まれた場合に、当サイトが報酬を受け取る仕組みとなっています。",
      "アフィリエイトリンクを経由しても、利用者に追加の費用が発生することはありません。当サイトは、利用者にとって有益な情報提供を第一に考え、広告主との関係によって記事内容が左右されることはありません。",
    ],
  },
  {
    title: "10. Cookieについて",
    paragraphs: [
      "当サイトでは、利便性の向上・アクセス解析・広告配信のためにCookieを使用しています。ブラウザの設定によりCookieを無効にすることが可能ですが、一部のサービスが利用できなくなる場合があります。",
    ],
  },
  {
    title: "11. アクセス解析ツールについて",
    paragraphs: [
      "当サイトではGoogle Analyticsを使用してアクセス解析を行っています。データは匿名で収集されており、個人を特定するものではありません。Cookieを無効にすることで収集を拒否できます。",
    ],
  },
  {
    title: "12. お問い合わせ",
