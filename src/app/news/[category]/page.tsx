...
export default function CategoryNewsPage({
  ...
  const isValidCategory = category in CATEGORY_MAP;

  const filteredArticles = isValidCategory
    ? allArticles.filter((a) => a.categoryKey === category)
    : allArticles;
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 pt-20 pb-12">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-gray-400 mb-6">
        ...
        <Link href="/news" className="hover:text-accent transition-colors">ニュース</Link>
        <ChevronRight className="h-3 w-3" />
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <h1 className="text-2xl font-bold text-navy">{categoryName}</h1>
      </div>
      {isValidCategory && (
        <p className="text-sm text-gray-400 mb-8">
          {categoryName}に関連する最新ニュースをAI要約でお届けします
        </p>
      )}
      {!isValidCategory && (
        <p className="text-sm text-gray-400 mb-8">
          該当するカテゴリが見つかりません。すべてのニュースを表示しています。
        </p>
      )}
      ...
      <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-1">
        <Link
          href="/news"
          className="px-4 py-2 text-sm font-medium rounded-lg whitespace-nowrap bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 transition-colors"
        >
          すべて
        </Link>
      ...
      {filteredArticles.length === 0 && (
        <div className="text-center py-16">
          ...
          <Link
            href="/news"
            className="inline-block mt-4 text-sm text-accent hover:text-accent-light transition-colors font-medium"
          >
            すべてのニュースに戻る &rarr;
          </Link>
        </div>
